    ...
    if (visible) {
        return (
            <div className={`flex flex-row w-full bg-border items-center justify-center p-2 px-4 subtitle gap-2 z-[39] ${fixedAtTop ? "fixed" : ""}`}>
                ...
                <button onClick={() => setVisible(false)} className="border-1 !border-transparent rounded focus:border-1 focus:!border-border-light focus-visible:outline-none transition-all p-1 ml-auto"><XMarkIcon className="w-5 h-5"/></button>
            </div>         
        );        
    }
