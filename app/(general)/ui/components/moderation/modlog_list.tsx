                    ...
                    {/*{Array.isArray(community.admin_ids) && community.admin_ids.map(async (admin_id) => {
                        ...
                        return (
                            <div key={admin_id} className="flex flex-row gap-4 w-full">
                                <div className="bg-card border-1 border-border p-4 rounded-lg flex flex-row gap-4 items-center w-full">
                                    <img src={user.image} className="w-6 h-6 rounded-lg-sm" alt={`Profile Picture of ${user.username}`} />
